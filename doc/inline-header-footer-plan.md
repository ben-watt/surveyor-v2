...
- **Templating defaults**: `app/home/editor/components/HeaderFooter.tsx` renders a single header table with `data-running-role="header"` plus an embedded address block (`data-running-role="address"`) and a standalone footer (`data-running-role="footer"`). These are emitted as plain strings that seed the inline editors.
- **Editor surface**: `app/home/components/Input/HeaderFooterEditor.tsx` hosts one TipTap instance per running region (`'header' | 'footer'`). It normalises markup into `.header-container` / `.footer-container` shells and enforces the legacy `headerPrimary/headerAddress/footerPrimary` structure.
- **State & autosave**: `app/home/components/Input/PageLayoutContext.tsx` stores a single `headerHtml` and `footerHtml`. `BlockEditor.tsx` exposes two popover editors wired to `setHeaderHtml` / `setFooterHtml` and passes those strings through the print payload.
...
- **Preview pipeline**: `app/home/editor/components/PrintPreviewer.tsx` expects a single concatenated HTML blob containing the running elements at the top level before body content; it loads `/pagedstyles.css` to resolve the margin boxes.
