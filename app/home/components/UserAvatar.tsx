 ...
 */
export function UserAvatar({ name, imageUrl, className, size = 'sm', alwaysColor = true }: UserAvatarProps) {
  const safeName = (name ?? '').trim() || 'Unknown';
  const initial = safeName[0]?.toUpperCase() || 'U';
  ...
  return (
    <Avatar className={[sizeClasses, alwaysColor ? colorClasses : '', className].filter(Boolean).join(' ')}>
