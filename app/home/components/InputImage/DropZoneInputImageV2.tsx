...
export const DropZoneInputImageV2 = ({
  ...
  const refreshFiles = useCallback(async (): Promise<void> => {
    try {
      ...
      const pathImages = Array.from(byId.values()).filter(
        (img) => img.imagePath.startsWith(path) && (img as any).isDeleted !== true,
      );
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: async (acceptedFiles: FileWithPath[]) => {
      ...
      const remaining =
        typeof maxFiles === 'number' ? Math.max(0, maxFiles - activeCount) : acceptedFiles.length;
      ...
      for (const originalFile of queue) {
        try {
          ...
          const dupResult = await enhancedImageStore.findDuplicate({
            contentHash,
            pathPrefix: path,
          });
          if (dupResult.ok && dupResult.val) {
            ...
            if ((existingImage as any).isDeleted) {
              ...
              if (replaceResult.ok) {
                toast.success(
                  `Replaced deleted image: ${existingImage.fileName ?? existingImage.imagePath}`,
                );
              } else {
            ...
            if (existingImage.isArchived) {
              ...
              if (restoreResult.ok) {
                toast(
                  `Restored archived image: ${existingImage.fileName ?? existingImage.imagePath}`,
                  { duration: 3000 },
                );
              } else {
              ...
              toast(`Image already exists: ${existingImage.fileName ?? existingImage.imagePath}`, {
                duration: 3000,
              });
            }
  ...
  const activeFilesOrdered = useMemo(() => {
    if (value && Array.isArray(value)) {
      ...
      const ordered = value.map((v) => byPath.get(v.path)).filter(Boolean) as DropZoneInputFile[];
  ...
  useEffect(() => {
    activeFilesOrderedRef.current = activeFilesOrdered;
  }, [activeFilesOrdered]);
  ...
  if (isLoading) {
    return (
      <div className="container rounded-md border border-gray-300 bg-gray-100 p-4">Loading...</div>
    );
  }

  return (
    <>
      <section className="container rounded-md border border-gray-300 bg-gray-100 p-4">
          ...
          {maxFiles !== activeFiles.length && (
            <div className="flex flex-col items-center justify-center">
              ...
              <p className="m-2 text-center text-sm text-gray-500">
                ...
                {' or '}
                <span className="cursor-pointer text-sm text-gray-500 underline hover:text-gray-700">
                  take photo
                </span>
              </p>
            </div>
        ...
        <aside>
          {enableReorder ? (
            ...
            >
              <SortableContext
                items={activeFiles.map((f) => f.path)}
