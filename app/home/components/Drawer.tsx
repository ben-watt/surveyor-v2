...
const MemoizedContent = React.memo(({ content }: { content: React.ReactNode }) => (
  <div>{content}</div>
));
...
const IndividualDrawer = React.memo(
  ({
    ...
    return <DynamicDrawer key={id} isOpen={true} handleClose={() => onClose(id)} {...props} />;
  },
);
IndividualDrawer.displayName = 'IndividualDrawer';

export function DynamicDrawerProvider({ children }: React.PropsWithChildren<{}>) {
  const [drawerStates, setDrawerStates] = React.useState<Record<string, DynamicDrawerOpenArgs>>({});
  ...
  return (
    <DynamicDrawerContext.Provider value={contextValue}>
      ...
      {Object.entries(drawerStates).map(([id, props]) => (
        <IndividualDrawer key={id} id={id} props={props} onClose={handleCloseDrawer} />
      ))}
  ...
  if (isDesktop) {
    return (
      <Dialog key={id} open={isOpen} onOpenChange={handleOpenChange}>
        <DialogContent className="max-h-full overflow-scroll sm:max-w-md">
          <DialogHeader>
