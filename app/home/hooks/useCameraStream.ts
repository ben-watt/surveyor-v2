...
export const useCameraStream = (): UseCameraStreamReturn => {
  ...
  const startCamera = useCallback(
    async (options: CameraStreamOptions = {}) => {
      if (!isSupported()) {
        setError('Camera API not supported in this browser');
        return;
      }
  ...
  const capturePhoto = useCallback(
    async (options?: CaptureOptions): Promise<Blob | null> => {
      ...
      if (!stream || !videoRef.current) {
        console.error('🔴 Missing stream or videoRef', {
          stream: !!stream,
      ...
      try {
        ...
        const previewScale =
          options?.previewZoomScale && options.previewZoomScale > 1 ? options.previewZoomScale : 1;
