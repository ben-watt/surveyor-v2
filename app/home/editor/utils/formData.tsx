  ...
  const mapElement = async (elementSection: ElementSection) => {
    ...
    const elementImages = await imageToImageWithMetadata(elementSection.images.filter((image: SurveyImage) => !image.isArchived));
    ...
    const components = await Promise.all(
      elementSection.components.map(async (component: any) => {
        ...
        const componentImages = await imageToImageWithMetadata(component.images.filter((image: SurveyImage) => !image.isArchived));
