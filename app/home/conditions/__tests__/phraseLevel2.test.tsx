...
const mockPhrases = [
  createMockPhrase('phrase-1', 'Crack in Wall', 'Technical description of crack in wall for Level 3 survey', 'Simple crack description for Level 2 survey'),
...
const filterPhrasesByLevel = (phrases: typeof mockPhrases, level: '2' | '3', componentId = 'component-1', elementId = 'element-1') => {
...
describe('Level 2 Phrase Feature', () => {
  describe('Phrase filtering logic', () => {
    ...
    it('should map correct phrase text based on survey level', () => {
      ...
      expect(getPhraseTextByLevel(phrase, '3')).toBe('Technical description of crack in wall for Level 3 survey');
    });

    it.each([
      { phraseIndex: 1, level: '2' as const, description: 'Level 3 only phrase should not be available for Level 2' },
      { phraseIndex: 1, level: '3' as const, description: 'Level 3 only phrase should be available for Level 3' },
      { phraseIndex: 2, level: '2' as const, description: 'Level 2 only phrase should be available for Level 2' },
      { phraseIndex: 2, level: '3' as const, description: 'Level 2 only phrase should not be available for Level 3' },
    ])('$description', ({ phraseIndex, level }) => {
      const phrase = mockPhrases[phraseIndex];
      const expectedAvailability = (phraseIndex === 1 && level === '3') || (phraseIndex === 2 && level === '2');
