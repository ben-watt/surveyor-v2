...
describe('useHierarchicalData', () => {
  ...
  describe('Data Structure Tests', () => {
    it('should build correct hierarchy from flat data', async () => {
      const { result, waitForNextUpdate } = renderHook(() => useHierarchicalData());
    ...
    it('should associate conditions with correct entities', async () => {
      ...
      expect(componentCondition?.data.associatedComponentIds).toContain('component-1');
      ...
      expect(elementCondition?.data.associatedComponentIds).toHaveLength(0);
      expect(elementCondition?.data.associatedElementIds).toContain('element-1');
    });
  ...
  describe('Data Integrity Tests', () => {
    it('should maintain referential integrity in tree structure', async () => {
      ...
      const foundationElement = structureSection.children[0];
      expect(foundationElement.data.sectionId).toBe('section-1');

      const concreteComponent = foundationElement.children.find(child => child.type === 'component');
      expect(concreteComponent?.data.elementId).toBe('element-1');

      const condition = concreteComponent?.children[0];
      expect(condition?.data.associatedElementIds).toContain('element-1');
      expect(condition?.data.associatedComponentIds).toContain('component-1');
    });
  ...
  describe('Performance Tests', () => {
    ...
    it('should memoize results to avoid unnecessary recalculations', async () => {
      const { result, rerender, waitForNextUpdate } = renderHook(() => useHierarchicalData());
