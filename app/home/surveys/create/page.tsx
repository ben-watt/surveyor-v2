  ...
  return {
    ...
    checklist: {
      items: [
        ...
        makeCheckbox('Have you checked external ground levels in relation to DPCs / Air Vents?'),
        makeCheckbox('Have you located services, elecs, gas, water, etc...?'),
        makeCheckbox('Have you checked if chimney breasts been removed internally?'),
        makeCheckbox('Have you checked the locations and severity of all cracks been logged?'),
        makeCheckbox(
  ...
  useEffect(() => {
    async function createSurvey() {
      try {
        ...
        const userName =
          userAttributes.name || userAttributes.given_name || user.username || 'Unknown User';
