...
const AddComponentButton = memo(
  ({ surveyId, elementId, elementName, imageUploadPath }: AddComponentButtonProps) => {
...
const ElementForm: React.FC<ElementFormProps> = ({ surveyId, sectionId, elementId }) => {
  ...
  const saveData = useCallback(
    async (data: ElementFormData, { auto = false } = {}) => {
      try {
  ...
  const { saveStatus, isUploading, lastSavedAt } = useAutoSaveFormWithImages(
    ...
    trigger,
    {
      ...createAutosaveConfigWithImages([imageUploadPath], { enabled: !isLoading }),
    ...
    <FormProvider {...methods}>
      <div className="space-y-6">
        <FormSection title="Element Details">
          ...
          />
          <div className="image-w-50 space-y-2">
