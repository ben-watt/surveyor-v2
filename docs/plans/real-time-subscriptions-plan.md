...
export class SubscriptionManager {
  ...
  private startSurveySubscriptions(tenantId: string) {
    ...
    const createSub = client.models.Surveys.onCreate({
    ...
    const updateSub = client.models.Surveys.onUpdate({
      filter: { tenantId: { eq: tenantId } },
    }).subscribe({
    ...
    const deleteSub = client.models.Surveys.onDelete({
      filter: { tenantId: { eq: tenantId } },
    }).subscribe({
...
function CreateDexieHooks<T extends TableEntity, TCreate, TUpdate>(
  ...
  const startSubscriptions = async () => {
    if (!subscriptionManager) {
      subscriptionManager = new SubscriptionManager(tableName, {
        onCreate: handleRemoteCreate,
